...
def _require_env(key: str) -> str:
    ...
    Never falls back to a hardcoded default for secrets.
    """
    raise ValueError(
        f"Missing required environment variable: {key}\n"
        f"Copy .env.example to .env and set a secure value."
    )
...
class AppConfig:
    ...
    # SECURITY FIX: Never use a hardcoded default password.
    # If NEO4J_PASSWORD is not set, the first DB operation will fail with
    # a clear message asking the operator to configure credentials.
